...
var primitives = require('swagger-parser-mock/lib/primitives')
  ...
  var spec = res.spec
    ...
    if (spec.swaggerVersion) { // v1
      var paths = spec.apis.map(function (api) {
        ...
        return swagger({url:URL.resolve(baseUrl, api.path.replace(/^\//, ''))})
      })
      return Promise.all(paths).then(function (apis:{[k:string]:any}) {
